...
const Engine = {
  ...
  loop() {
    ...
    // Throttle solve to every 2 frames (~15fps at 30fps cap) — electrical quantities don't need high-rate updates
    this._solveFrame = (this._solveFrame || 0) + 1;
    if (this._solveFrame >= 2) { this._solveFrame = 0; this.solve(); }
    // requestRender now handles 30fps throttling internally
    requestRender();
    this.simLoop = requestAnimationFrame(() => this.loop());
  },
  ...
  solve() {
    ...
    // Track visual state changes — only rebuild static layer when component appearance changes
    // (LED lit/unlit, relay energized, fault states) — not every frame
    const visualHash = S.components.map(c =>
      c.type + '_' + (c.simCurrent > 0 ? '1' : '0') + '_' + (c._fault || '') + '_' + (c.props.energized ? 'E' : 'D') + '_' + (c.props.closed ? 'C' : 'O') + '_' + (c.props.pressed ? 'P' : 'R')
    ).join('|');
    if (this._lastVisualHash !== visualHash) {
      this._lastVisualHash = visualHash;
      markStaticDirty();
    }
    } catch(err) {
