...
const WireRouter = {
  ...
  WireColors: (typeof WireStyle !== 'undefined' && WireStyle.colors) ? WireStyle.colors : { live:'#e53935', neutral:'#1e88e5', ground:'#43a047', signal:'#ffa800', dc_pos:'#ff6d00', dc_neg:'#00bcd4', purple:'#ab47bc', cyan:'#00bcd4', pink:'#ec407a', gold:'#ffc107' },
  WireWidth: (typeof WireStyle !== 'undefined') ? WireStyle.width : 5,

  isActive() { return this.active; },

  // Shared wire endpoint resolver (used by drawWires and current flow overlay)
  _getWireEndpoints(w) {
    let p1, p2;
    if (w.from.junction) {
      p1 = w.from.junction;
  ...
  setWireType(type) {
    this.wireType = type;
    const colorMap = this.WireColors;
  ...
  drawWires() {
    ...
    S.wires.forEach(w => {
      const eps = this._getWireEndpoints(w);
      if (!eps) return;
      const [p1, p2] = eps;
      const allPoints = [p1, ...(w.waypoints || []), p2];
      ...
      // Current flow (skip during static layer — drawn as dynamic overlay)
      if (!_drawingStatic && S.simRunning && S.showCurrentDir && w.current > 0) {
  ...
  drawCurrentFlow(points, current, wireColor, wire) {
    ...
    if (isDC) {
      ...
      const dcDl = (typeof WireStyle !== 'undefined' && WireStyle.flow) ? WireStyle.flow.dc.dashLen : 8;
      const dcGl = (typeof WireStyle !== 'undefined' && WireStyle.flow) ? WireStyle.flow.dc.gapLen : 12;
      const dashLen = dcDl, gapLen = dcGl;
      ctx.beginPath();
      ctx.moveTo(flowPoints[0].x, flowPoints[0].y);
      for (let i = 1; i < flowPoints.length; i++) ctx.lineTo(flowPoints[i].x, flowPoints[i].y);
      ctx.strokeStyle = flowColor;
      ctx.setLineDash([dashLen, gapLen]);
      ctx.lineDashOffset = -(S.animTick * speed * dcSpeedMul);
      ctx.globalAlpha = 0.8;
      ctx.stroke();
      ctx.setLineDash([]);
    } else {
      const acDl = (typeof WireStyle !== 'undefined' && WireStyle.flow) ? WireStyle.flow.ac.dashLen : 12;
      const acGl = (typeof WireStyle !== 'undefined' && WireStyle.flow) ? WireStyle.flow.ac.gapLen : 20;
      const dashLen = acDl, gapLen = acGl;
      ctx.beginPath();
      ctx.moveTo(flowPoints[0].x, flowPoints[0].y);
      for (let i = 1; i < flowPoints.length; i++) ctx.lineTo(flowPoints[i].x, flowPoints[i].y);
      ctx.strokeStyle = flowColor;
      ctx.setLineDash([dashLen, gapLen]);
      ctx.lineDashOffset = -(S.animTick * speed * acSpeedMul);
      ctx.stroke();
      ctx.setLineDash([]);
    }
    ctx.restore();
  ...
  _drawFlowingArrows(points, speed, isDC) {
    ...
    const flowCfg = (typeof WireStyle !== 'undefined' && WireStyle.flow) ? WireStyle.flow : null;
    const arrowSpacing = isDC ? (flowCfg ? flowCfg.dc.arrowSpacing : 25) : (flowCfg ? flowCfg.ac.arrowSpacing : 35);
    const dcSpeedMul = flowCfg ? flowCfg.dc.speed : 0.25;
    const acSpeedMul = flowCfg ? flowCfg.ac.speed : 0.18;
    ...
    const t = (S.animTick * speed * (isDC ? dcSpeedMul : acSpeedMul)) % cycleLen;
    ...
    for (let i = 0; i < numArrows; i++) {
      ...
      if (isDC) {
        ...
        const dcAs = (typeof WireStyle !== 'undefined' && WireStyle.flow) ? WireStyle.flow.dc.arrowSize : 5;
        const arrowSize = dcAs;
        ...
        const acAs = (typeof WireStyle !== 'undefined' && WireStyle.flow) ? WireStyle.flow.ac.arrowSize : 6;
        const arrowSize = acAs;
        const goldR = 255, goldG = 210, goldB = 0;
