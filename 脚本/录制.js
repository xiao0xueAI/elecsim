...
const Recorder = {
  ...
  format: 'mp4',
  fps: 30,
  qualityMap: { high: 12000000, medium: 6000000, low: 2000000 },
  quality: 12000000,
  ...
  // scale: export multiplier (1=screen, 2=HD, 3=print). Renders at full DPR × scale.
  captureImage(scale) {
    scale = scale || 2;  // default 2× HD PNG
    if (this.isRecording) {
      UI.toast('录制中无法截图，请先停止录制', 'error');
      return;
    }
    const dpr = window.devicePixelRatio || 1;
    // Render the scene into an offscreen canvas at DPR × scale resolution
    // (real re-render, NOT a blurry upscale) so product wiring diagrams stay crisp.
    const off = document.createElement('canvas');
    off.width  = Math.max(1, Math.round(W * dpr * scale));
    off.height = Math.max(1, Math.round(H * dpr * scale));
    const octx = off.getContext('2d');
    octx.scale(dpr * scale, dpr * scale);   // logical coordinate space stays W×H

    // Temporarily hand the global ctx to the offscreen canvas so Renderer
    // draws the white-bg scene directly into it.
    const savedCtx = ctx;
    const savedSel = S.selected;
    ctx = octx;
    S.recording = true;   // white background, no grid
    S.selected = null;    // product diagram: no selection outline
    try {
      Renderer.render();
    } finally {
      S.recording = false;
    ...
    try {
      ...
      this._showImagePreview(dataUrl, off.width, off.height);
    } catch(e) {
      UI.toast('截图失败: ' + e.message, 'error');
    }
  },

  _showImagePreview(dataUrl, w, h) {
    ...
    img.src = dataUrl;
    dlBtn.href = dataUrl;
    dlBtn.download = 'ElecSim_' + new Date().toISOString().slice(0, 19).replace(/:/g, '-') + '.png';
    if (info && w && h) {
      info.textContent = '白底 PNG · ' + w + ' × ' + h + ' px · 适用于产品接线图';
    }

    modal.classList.add('show');
    UI.toast('截图完成，可下载或关闭', 'success');
  },
  ...
  downloadImage() {
    // The download link's click is handled by the <a> tag itself
    this.closeImagePreview();
    UI.toast('图片已导出', 'success');
  },
