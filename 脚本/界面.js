  ...
  showProps(comp) {
    ...
    const makeRow = (label, key, val, type = 'number') => {
      ...
      row.querySelector('input').addEventListener('change', e => {
        ...
        if (c) { c.props[key] = type === 'number' ? parseFloat(e.target.value) : e.target.value; S.dirty = true; markStaticDirty(); }
      });
    ...
    posGroup.innerHTML = '<div class="prop-group-title">📍 位置与尺寸</div>';
    ['x', 'y', 'w', 'h'].forEach(key => {
      const labels = { x: 'X 坐标', y: 'Y 坐标', w: '宽度 (px)', h: '高度 (px)' };
      const row = document.createElement('div'); row.className = 'prop-row';
      row.innerHTML = `<span class="label">${labels[key]}</span><input type="number" value="${Math.round(comp[key] || 0)}">`;
      row.querySelector('input').addEventListener('change', e => {
        const c = getComp(comp.id);
        if (c) { c[key] = parseFloat(e.target.value) || 0; S.dirty = true; markStaticDirty(); }
        // 实时刷新输入框显示（其他行可能因引用变化）
        if (key === 'w' || key === 'h') this.showProps(comp);
      });
    ...
    if (comp.type === 'resistor') makeRow('阻值 (Ω)', 'resistance', comp.props.resistance);
      ...
      row.querySelector('input').addEventListener('input', e => { comp.props.color = e.target.value; markStaticDirty(); });
      content.appendChild(row);
    }
      ...
      row.querySelector('button').addEventListener('click', () => { comp.props.closed = !comp.props.closed; S.dirty = true; this.showProps(comp); markStaticDirty(); });
      content.appendChild(row);
    }
      ...
      row.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', () => { comp.props.position = parseInt(btn.dataset.pos); S.dirty = true; this.showProps(comp); markStaticDirty(); });
      });
      content.appendChild(row);
    }
      ...
      row.querySelector('button').addEventListener('click', () => { comp.props.pressed = !comp.props.pressed; S.dirty = true; this.showProps(comp); markStaticDirty(); });
      content.appendChild(row);
    }
      ...
      if (comp.type === 'dry_relay') {
        ...
        modeRow.querySelector('button').addEventListener('click', () => {
          ...
          S.dirty = true; this.showProps(comp); markStaticDirty();
        });
      ...
      row.querySelector('button').addEventListener('click', () => { comp.props.energized = !comp.props.energized; S.dirty = true; this.showProps(comp); markStaticDirty(); });
      content.appendChild(row);
    }
